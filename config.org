...
2. If current buffer is Dired: use the file at point and its disk contents.
3. Otherwise: signal an error."
    (cond
     ...
     ;; Case 2: Dired buffer — file at point
...
- In a Dired buffer: operates on the file at point.
...
replaces the old prefix with the new one."
    (interactive)
    (when-let* ((resolved (or (and content file (cons file content))
                              (fanshi/gptel-agent--resolve-file-and-content)))
                (file (car resolved))
