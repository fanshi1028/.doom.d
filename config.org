...
#+begin_src emacs-lisp :tangle yes
(advice-add #'gptel-send :after (lambda (&rest r) "call `keyboard-quit' after, to temp fix an unexpected trigger of snippet by org-cycle calls in reponse" (if (use-region-p) (keyboard-quit))))
#+end_src
** Agent
#+begin_src emacs-lisp :tangle no
