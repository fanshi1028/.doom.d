        ...
        (let ((data (json-parse-buffer :object-type 'plist)))
          (mapcar (lambda (model)
                    (let ((id (plist-get model :id)))
                      ;; Keep both path and suffix parts of the model name
                      (intern id)))
                  (plist-get data :data))))))

  (unless (bound-and-true-p fanshi/gptel--llama-models)
    (setq fanshi/gptel--llama-models (fanshi/gptel--fetch-llama-server-models)))
...
(use-package! agent-shell
  :config
  (setq! agent-shell-opencode-acp-command (append '("nono" "run" "--profile" "opencode" "--allow-cwd" "--") agent-shell-opencode-acp-command)))
#+end_src
* Use spatial-window
#+begin_src emacs-lisp :tangle yes
