...
#+begin_src emacs-lisp :tangle yes
(after! gptel
  (defun fanshi/gptel--validate-simple-tool-arg-type (type arg optional)
    (or
     (pcase type
            ...
            (mapcar
             (lambda (arg)
               (let* ((name (plist-get arg :name))
                 ...
                 (and
                  (cond
                   ((not (fanshi/gptel--validate-simple-tool-arg-type type input_arg optional)) simple-error)
                   ((and (eq type 'array) (not (listp input_arg))) simple-error)
                   ((eq type 'array) (let ((items (plist-get arg :items)))
                                       (if (and items (all (lambda (item) (not (fanshi/gptel fs--validate-simple-tool-arg-type item input_arg optional))) items))
                                           (format "Error: tool call arg %s (%s) expected array with item type in %s but got %s\n\n" name description items input_arg)
                                         nil)))
                   ;; NOTE: :properties, if the type is object.  Its value must be a plist that can be
                   ;; serialized into a JSON object specification by `json-serialize'.
                   ((eq type 'object) nil)) ;; TEMP FIXME: complex validation of object is not needed, as I don't have a tool with object arg yet, so skip it for now
                  (when (and enum (member input_arg enum)) (format "Error: tool call arg %s (%s) expected enum (in %s) but got %s\n\n" name description enum input_arg)))))
...
COMMAND is the bash command string to execute."
    (if-let* ((output-buffer (generate-new-buffer " *gptel-agent-bash*"))
