...
(after! org-noter
  ...
  (setq! org-noter-notes-search-path (list (concat org-directory "/noter/"))
         org-noter-find-additional-notes-functions '((lambda (filename) (concat (file-name-base filename) ".org.gpg")))
...
(after! org
  (setq! fanshi/alltodo
         `((:discard (:scheduled t :deadline t :regexp ,org-scheduled-time-hour-regexp :file-path ("routine" "inbox" "work")))
