...
(after! gptel
  (setq!
   ...
   gptel-backend (gptel-make-gemini "Gemini"
                   :key (lambda () (+pass-get-secret "Google/1/gemini_key"))
   ...
   gptel-directives (cons '(full . "Please think harder to give your highest quality answer.
    ...
    Please show us your cognitive limits.") gptel-directives))
  (gptel-make-preset 'full :model 'gemini-flash-latest :backend "Gemini" :system 'full)
...
** Agent
#+begin_src emacs-lisp :tangle no
(after! gptel (use-package! gptel-agent))
#+end_src
...
"
  (let ((results '())
        (start 0)
        ;; A robust regex for common YouTube URL formats:
        ;; 1. Optional protocol (http/https)
        ;; 2. Optional www.
        ;; 3. Core domains: youtube.com or youtu.be
        ;; 4. Paths: /watch?v=, /shorts/, /embed/, or just / (for youtu.be)
        ;; 5. Video ID: 11 characters (alphanumeric, hyphen, underscore)
        ;; 6. Remaining URL part: captures query parameters like &list=
        (youtube-regexp
         (rx (or
    ...
    (while (string-match youtube-regexp string start)
      ;; Match data is set by string-match.
      ;; (match-string 0) returns the entire matched string (the full URL).
      (push (match-string 0 string) results)

      ;; Advance the search position to continue search after the current match.
      ;; This prevents an infinite loop if the regex can match an empty string,
      ;; and ensures we find non-overlapping matches.
      (setq start (match-end 0)))

    ;; The links are pushed in reverse order, so we reverse the list for correct order.
    (nreverse results)))
    #+end_src

*** Test case
#+begin_src elisp :tangle no :results code replace
...
#+RESULTS:
#+begin_src elisp
("https://www.youtube.com/watch?v=dQw4w9WgXcQ" "https://youtu.be/kU9ZcO4w38o"
 "http://youtube.com/watch?v=zT6")
#+end_src
