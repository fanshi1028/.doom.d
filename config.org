...
(after! org
  (setq! org-todo-keywords
         '((sequence "TODO(t)" "PROJ(p)" "WAIT(w@)" "HOLD(h@)" "IDEA(i)" "|" "DONE(d)" "KILL(k@)")
           (sequence "[ ](T)" "[?](W@)" "|" "[X](D)")
           (sequence "INBOX" "|" ))))
#+end_src
** Todo Keywords Faces
...
(after! org
  (setq! fanshi/alltodo
         `((:discard (:scheduled future :deadline future :regexp ,org-scheduled-time-hour-regexp :file-path ("routine" "inbox" "work")))
           ...
           ;; (:discard (:habit t))
           ;; NOTE Project
           (:discard (:not (:file-path "projects") :todo "PROJ"))
...
(after! org
  (setq! fanshi/worktodo
         `((:discard (:scheduled future :deadline future :regexp ,org-scheduled-time-hour-regexp :not (:file-path ("work"))))
           (:name "Important 💎💎💎" :tag "Payment" :priority "A" :order 2) ;;🚔🚔🚔
           (:name "Camping 🏕🏕🏕" :todo "WAIT" :order 11) ; Set order of this section 💎💎💎
           (:name "Quick Picks 🚀🚀🚀" :and (:effort< "0:15" :todo "TODO") :order 4)
           (:name "Super B 👶👶👶" :and (:priority "B") :order 6)
           (:name "Others 🏝🏝🏝" :and (:priority "C") :order 21)
           (:auto-outline-path t  :order 7))))
#+end_src
...
(use-package! org-super-agenda
  ...
  (setq org-agenda-show-log t
        ...
        org-agenda-custom-commands '(("p" "Private Agenda"
                                     ...
                                     ("w" "Work Agenda"
                                      ((agenda "" ((org-super-agenda-groups fanshi/agenda)
                                                   (org-agenda-file-regexp fanshi/org-work-agenda-file-regexp)))
                                       (alltodo "" ((org-agenda-overriding-header (fanshi/make-line))
                                                    (org-super-agenda-groups fanshi/worktodo)
                                                    (org-agenda-file-regexp fanshi/org-private-agenda-file-regexp)))))))
  :config
...
** Journal Encrypt
#+begin_src emacs-lisp :tangle yes
(after! org-journal (unless (string-suffix-p ".gpg" org-journal-file-format)
                      (setq! org-journal-file-format (concat org-journal-file-format ".gpg"))))
#+end_src
