...
*** Local llama-server
#+begin_src emacs-lisp :tangle yes
(after! gptel (defun fanshi/gptel--parse-llama-ini ()
                         ...
                         ((string-match "^\\([a-z-]+\\)\\s-*=\\s-*\\(.*\\)$" line)
                          (let ((key (intern (concat ":" (match-string 1 line))))
                            ...
                            (setq val (cond
                                       ...
                                       ((string= "nil" val) nil)
                                       (t val)))
                            (push (cons key val) current-props)))
  ...
  ;; Model multimodal capabilities based on research
  (defvar fanshi/gptel--llama-ini-models-multimodal
    '((Qwen3.5-27B . ((capabilities media tool-use json url)
                      (mime-types "image/jpeg" "image/png" "image/gif" "image/webp")))
      (Qwen3.5-27B-Claude-4.6-Opus-Reasoning-Distilled . ((capabilities reasoning)
                                                          (mime-types)))
      (Qwen3.5-122B-A10B . ((capabilities media tool-use json url)
                            (mime-types "image/jpeg" "image/png" "image/gif" "image/webp")))
      (Qwen3.5-35B-A3B . ((capabilities media tool-use json url)
                          (mime-types "image/jpeg" "image/png" "image/gif" "image/webp")))
      (GLM-4.7-Flash . ((capabilities media tool-use json url)
                        (mime-types "image/jpeg" "image/png" "image/gif" "image/webp")))
      (gpt-oss-120b . ((capabilities tool-use json)
                       (mime-types)))
      (NVIDIA-Nemotron-3-Super-120B-A12B . ((capabilities tool-use json)
                                            (mime-types)))
      (Nemotron-3-Nano-30B-A3B . ((capabilities tool-use json)
                                  (mime-types)))
      (Qwen3-Coder-Next . ((capabilities tool-use json)
                           (mime-types)))
      (OmniCoder-9B . ((capabilities tool-use json)
                       (mime-types)))
      (WEBGEN-OSS-20B . ((capabilities)
                         (mime-types)))
      ;; Uncensored variants inherit from base Qwen3.5
      (Qwen3.5-122B-A10B-Uncensored-HauhauCS-Aggressive . ((capabilities media tool-use json url)
                                                           (mime-types "image/jpeg" "image/png" "image/gif" "image/webp"))))
    "Alist of model multimodal capabilities for INI models.")

  (defun fanshi/gptel--get-model-mime-types (model)
    "Get MIME types for MODEL from multimodal capabilities list."
    (when-let ((caps (cdr (assoc model fanshi/gptel--llama-ini-models-multimodal))))
      (cdr (assoc 'mime-types caps))))

  (defun fanshi/gptel--get-model-capabilities (model)
    "Get capabilities for MODEL from multimodal capabilities list."
    (when-let ((caps (cdr (assoc model fanshi/gptel--llama-ini-models-multimodal))))
      (cdr (assoc 'capabilities caps))))
  ...
  (defun fanshi/gptel--model-to-gptel-spec (model-name)
    ...
    (when-let ((props (cdr (assoc model-name (fanshi/gptel--parse-llama-ini))))
               (caps (fanshi/gptel--get-model-capabilities model-name))
               (mime-types (fanshi/gptel--get-model-mime-types model-name)))
      `(,model-name
  ...
  (defvar fanshi/gptel--llama-ini-model-specs nil
    "List of model specifications from INI file.")

  (defun fanshi/gptel--build-llama-model-specs ()
    "Build gptel model specifications from INI file."
    (setq fanshi/gptel--llama-ini-model-specs
          (mapcan (lambda (model)
  ...
  ;; Initialize
  (fanshi/gptel--build-llama-model-specs)

  ;; Set up gptel backend with INI models
  (setopt
   ...
   (gptel-make-openai "Local Llama"
     ...
     :protocol "http"
     :stream t
     :models fanshi/gptel--llama-ini-model-specs)))
#+end_src
