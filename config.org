...
#+begin_src emacs-lisp :tangle yes
(after! gptel
  (defun fanshi/gptel--fetch-llama-server-models ()
    "Fetch model list from local llama-server."
    (require 'url)
    (let ((url-automatic-caching nil)
          (url-request-method "GET")
          (url-request-extra-headers '(("Content-Type" . "application/json"))))
      (with-current-buffer (url-retrieve-synchronously "http://localhost:8080/v1/models"
                                                       nil nil 5)
        (goto-char url-http-end-of-headers)
        (let ((data (json-parse-buffer :object-type 'plist)))
          (defconst fanshi/gptel--llama-models
            (mapcar (lambda (model) (intern (plist-get model :id)))
                    (plist-get data :data)))))))

  (setopt
   gptel-model 'Qwen3-Coder-Next-GGUF:Q8_0
   gptel-backend
   (gptel-make-openai "Local Llama"
     :host "127.0.0.1:8080"
     :protocol "http"
     :stream t
     :models (prog2 (or (boundp 'fanshi/gptel--llama-models)
                        (fanshi/gptel--fetch-llama-server-models))
                 fanshi/gptel--llama-models)
