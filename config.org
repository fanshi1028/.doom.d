...
#+end_src
*** fanshi/gptel-agent--write-file-a
#+begin_src emacs-lisp :tangle yes
(after! gptel
  (defun fanshi/gptel-agent--write-file-a (&rest args)
    (if-let* ((root (projectile-project-root))
              (abs-root (expand-file-name root))
              (path (car args))
              (filename (cadr args))
  ...
  (advice-add 'gptel-agent--write-file :before #'fanshi/gptel-agent--write-file-a))
#+end_src
