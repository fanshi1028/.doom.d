...
*** Local
**** common
***** fanshi/gptel--parse-ini
#+begin_src emacs-lisp :tangle yes
(after! gptel
  (defun fanshi/gptel--parse-ini (ini-file)
    "Parse INI-FILE and return model configurations as alist of (symbol . plist).
INI-FILE is the filename (without path) to parse from doom-user-dir."
    (let ((models '()) (section nil) (props '()))
      (with-temp-buffer
        (insert-file-contents-literally (expand-file-name ini-file doom-user-dir))
        (goto-char (point-min))
        (while (not (eobp))
          (let ((line (buffer-substring-no-properties
            ...
            (cond
             ...
             ((string-match "^\\[\\([^]]+\\)\\]$" line)
              (when section (push (cons section (nreverse props)) models))
              (setq section (intern (match-string 1 line)) props '()))
             ...
             ((string-match "^\\([a-z-]+\\)\\s-*=\\s-*\\(.*\\)$" line)
              (let ((key (intern (concat ":" (match-string 1 line))))
                ...
                (setq val (cond
                           ...
                           ((string= "nil" val) nil)
                           (t val)))
                (push key props) (push val props)))
             ((not (string-match-p "^\\s*[;\\s-]*$" line)) nil))
            (forward-line 1)))
        (when section (push (cons section (nreverse props)) models)))
      (nreverse models))))
#+end_src
...
(after! gptel
  (defun fanshi/gptel--infer-capabilities-from-hf (hf-tag)
    "Infer capabilities based on HF TAG.
Returns a cons cell of (CAPABILITIES MIME-TYPES)."
    (let ((tag-lower (downcase hf-tag)))
      (cond
       ;; Multimodal models (Qwen3.5, GLM-4.6/4.7, gpt-oss)
       ((string-match-p "\\(qwen3\\.5\\|glm-4\\.6\\|glm-4\\.7\\|gpt-oss\\)" tag-lower)
        (cons '(media tool-use json url) '("image/jpeg" "image/png" "image/gif" "image/webp")))
       ;; Coding models (-coder, omnicoder)
       ((string-match-p "\\(-coder\\|omnicoder\\)" tag-lower)
        (cons '(tool-use json) '()))
...
#+begin_src emacs-lisp :tangle yes
(after! gptel
  (defun fanshi/gptel--model-to-gptel-spec (model-name props)
    "Convert MODEL-NAME and PROPS to gptel model specification with capabilities."
    (let* ((hf-tag (or (plist-get props :hf) ""))
           (caps-mime (fanshi/gptel--infer-capabilities-from-hf hf-tag))
           (caps (car caps-mime))
           (mime-types (cdr caps-mime))
           (desc (plist-get props :description))
           (ctx (or (plist-get props :ctx-size) 65536))
           (temp (plist-get props :temp))
           (top-p (plist-get props :top-p))
           (top-k (plist-get props :top-k))
           (min-p (plist-get props :min-p))
           (presence (plist-get props :presence-penalty)))
      (list model-name
            :description (or desc "Local model")
            :capabilities caps
            :mime-types mime-types
            :context-window ctx
            :input-cost 0
            :output-cost 0
            :request-params (nconc (when temp (list :temperature temp))
                                   (when top-p (list :top_p top-p))
                                   (when top-k (list :top_k top-k))
                                   (when min-p (list :min_p min-p))
                                   (when presence (list :presence_penalty presence)))))))
#+end_src
***** fanshi/gptel--load-models
#+begin_src emacs-lisp :tangle yes
(require 'cl-lib)

(after! gptel
  (defun fanshi/gptel--load-models (ini-file)
    "Build gptel model specifications from INI file."
    (let ((models (fanshi/gptel--parse-ini ini-file)))
      (cl-mapcar #'fanshi/gptel--model-to-gptel-spec
                 (mapcar #'car models)
                 (mapcar #'cdr models)))))
#+end_src
**** llama-server
...
(after! gptel
  (setopt
   gptel-model 'Qwen3.5-122B-A10B
   gptel-backend
   (gptel-make-openai "Local Llama"
     :host "127.0.0.1:8080"
     :protocol "http"
     :stream t
     :models (fanshi/gptel--load-models "llama-model.ini"))))
#+end_src
...
(after! gptel
  (setopt
   ...
   (gptel-make-openai "Local MLX"
     ...
     :protocol "http"
     :stream t
     :models (fanshi/gptel--load-models "mlx-model.ini"))))
#+end_src
