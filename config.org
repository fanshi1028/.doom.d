...
#+begin_src emacs-lisp :tangle yes
(setq! org-id-locations-file "~/Documents/org/.orgids.gpg")
#+end_src
...
- REF :: https://github.com/emacs-lsp/lsp-mode/issues/713#issuecomment-2646367215
#+begin_src emacs-lisp :tangle no
(after! lsp-mode
  (advice-add 'lsp--get-ignored-regexes-for-workspace-root
              :around (lambda (fn workspace-root)
                        (let* ((ignored-things (funcall fn workspace-root))
                               ...
                               (cmd (format
                                     "git clean --dry-run -X '%s' | cut -d' ' -f3"
                                     workspace-root))
                               ...
                               (gitignored-files-regex-list (mapcar (lambda (file) (concat "[/\\\\]" (regexp-quote file) "\\'")) gitignored-files))
                               (gitignored-directories-regex-list (mapcar (lambda (directory) (concat "[/\\\\]" (regexp-quote (replace-regexp-in-string "[/\\\\]\\'" "" directory)) "\\'")) gitignored-directories)))
                          (list (append ignored-files-regex-list gitignored-files-regex-list) (append ignored-directories-regex-list gitignored-directories-regex-list))))))
#+end_src
** Emacs lsp booster
- ref :: https://github.com/blahgeek/emacs-lsp-booster/tree/fd4301350c41fe954d25ed663078af520902d87c
#+begin_src emacs-lisp :tangle no
(defun fanshi/lsp-booster--advice-final-command (old-fn cmd &optional test?)
  "Prepend emacs-lsp-booster command to lsp CMD."
  (let ((orig-result (funcall old-fn cmd test?)))
    (if-let* ((booster (and (not test?) ;; for check lsp-server-present?
                            (not (file-remote-p default-directory)) ;; see lsp-resolve-final-command, it would add extra shell wrapper
                            ;; lsp-use-plists
                            (not (functionp 'json-rpc-connection)) ;; native json-rpc
                            (seq-some (lambda (path) (and (string-match ".*emacs-lsp-booster.*" path) (concat path "/emacs-lsp-booster"))) (default-value 'exec-path)))))
        (progn
          (message "Using emacs-lsp-booster for %s!" orig-result)
          (append (list booster "-n" "--") orig-result))
      orig-result)))
(advice-add 'lsp-resolve-final-command :around #'fanshi/lsp-booster--advice-final-command)
#+end_src
...
* Use Gif Screencast
#+begin_src emacs-lisp :tangle no
;; FIXME maybe reenable it, and set better keys?
(use-package! gif-screencast
  :defer t
  :commands (gif-screencast-start-or-stop)
  :config (setq gif-screencast-args '("-x")
...
#+begin_src emacs-lisp :tangle yes
(after! gptel
  (gptel-make-gemini "Gemini" :key (lambda () (+pass-get-secret "Google/1/gemini_key")) :stream t)
  (setq!
   gptel-model 'gemini-flash-lite-latest
   gptel-display-buffer-action t
   gptel-default-mode 'org-mode
   gptel-backend (gptel-get-backend "Gemini"))
  ;; remove unfree/old/duplicated models
  (->> gptel--gemini-models
       (assq-delete-all 'gemini-pro-latest)
       (assq-delete-all 'gemini-3-pro-preview)
       (assq-delete-all 'gemini-2.5-pro)
       (assq-delete-all 'gemini-2.5-pro-preview-06-05)
       (assq-delete-all 'gemini-2.5-flash)
       (assq-delete-all 'gemini-2.5-flash-preview-09-2025)
       (assq-delete-all 'gemini-2.5-flash-lite-preview-09-2025)
       (assq-delete-all 'gemini-2.0-flash)
       (assq-delete-all 'gemini-2.0-flash-exp)
       (assq-delete-all 'gemini-2.0-flash-thinking-exp))
  (gptel-make-gh-copilot "Copilot"))
#+end_src
** Agent
#+begin_src emacs-lisp :tangle yes
(after! gptel (use-package! gptel-agent :config (gptel-agent-update)))
#+end_src
** Preset for Gemini Builtin Tools
#+begin_src emacs-lisp :tangle yes
;; NOTE: https://github.com/google/adk-docs/tree/82e9fa553fc42d0ddc8ebca9b0b0ed17b1621fa3/docs/tools/built-in-tools.md#limitations
(after! gptel
  (gptel-make-preset 'google_search
    :description  "Agent who do google search"
    :backend      "Gemini"
    :model        'gemini-flash-lite-latest
    :request-params '(:tools [(:google_search ())]))
  (gptel-make-preset 'google_maps
    :description  "Agent who can access google map"
    :backend      "Gemini"
    :model        'gemini-2.5-flash-lite
    :request-params '(:tools [(:googleMaps ())]
                      ;; :toolConfig (:retrievalConfig (:latLng (:latitude 34.050481 :longitude -118.2))
                      ))
  (gptel-make-preset 'code_execution
    :description  "Gemini with basic web search capability."
    :backend      "Gemini"
    :model        'gemini-flash-lite-latest
    :request-params '(:tools [(:code_execution ())]))
  (gptel-make-preset 'url_context
    :description  "Gemini with basic web search capability."
    :backend      "Gemini"
    :model        'gemini-flash-lite-latest
    :request-params '(:tools [(:url_context ())])))
#+end_src
